...
simulate_data_sick <- function(rdata_path = "mom_270.Rdata",
  ...
  sick_time_options <- as.matrix(expand.grid(rep(list(0:1),m)))
  # sick_time <- t(sick_time_options[sample(nrow(sick_time_options), n, replace = TRUE),])
  
  prob0 <- rep(1, 16) 
  prob1 <- rep(1, 16)
  
  # Penalize patterns with lots of 1s under treatment
  num_ones <- rowSums(sick_time_options)            
  prob1 <- exp(-0.6 * num_ones)   # fewer ones - higher prob
  prob1 <- prob1 / sum(prob1)
  
  # Control slightly favors "getting sick over time"
  late_sick_score <- sick_time_options[,4] + 0.5*sick_time_options[,3]
  prob0 <- exp(0.3 * late_sick_score)
  prob0 <- prob0 / sum(prob0)
  
  batchid <- matrix(0, nrow = m, ncol = n)
  cond <- rbinom(n, 1, 0.5) # treatment/control
  
  for (i in 1:n) {
    idx <- if (cond[i] == 1) {
      sample.int(16, size = 1, prob = prob1)
    } else {
      sample.int(16, size = 1, prob = prob0)
    }
    batchid[, i] <- sick_time_options[idx, ]
  }
